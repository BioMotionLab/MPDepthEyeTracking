...
public class RotatableDeviceTrackingCalibrationProvider : TrackingCalibrationProvider
{
    ...
    protected override void LoadSelfFromJson(string json)
    {
        ...
        savedCalibrations = loadedData.savedCalibrations;
        currentCalibration = loadedData.currentCalibration;
    }

    protected override string GetSelfAsJson()
    {
        SaveData saveData = new SaveData(currentCalibration, savedCalibrations);
    ...
    public override async Task StartCalibration()
    {
        ...
        currentCalibration = new SavedRotatableDeviceCalibration();
        bool success = await rotatableCameraCalibrator.RunCalibrationProcedure(currentCalibration);
    ...
    {
        allLoadedCalibrations = new List<SavedRotatableDeviceCalibration>();
        List<SavedRotatableDeviceCalibration> defaults =
            defaultCalibrations.Select(calibrationAsset => calibrationAsset.trackingCalibration).ToList();
        allLoadedCalibrations.AddRange(savedCalibrations);
        allLoadedCalibrations.AddRange(defaults);
    }
